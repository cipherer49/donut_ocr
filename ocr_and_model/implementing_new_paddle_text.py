...
# Default paths for images and text output
default_img_path = "data/page_15.jpg"
ocr_text_path = 'data/page15_output.txt'
...
class AllFunc:
    ...
    def dump_in_text(self, file_path):
        # Get the most frequent class
        most_frequent_class = self.class_counter.most_common(1)[0][0]

        # Prepare the final text output
        output_lines = [f"Class: {most_frequent_class}", ""] + self.ocr_lines

        with open(file_path, 'w') as file:
            file.write("\n".join(output_lines))
            print(f"Saved in Text: {file_path}")
    ...
    def process_images(self):
        if os.path.isdir(self.img_path):
            ...
            # Dump all results into a single text file
            self.dump_in_text(ocr_text_path)
